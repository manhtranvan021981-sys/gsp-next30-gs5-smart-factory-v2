...
def main() -> int:
    ...
    no_khay_inference = raw_row(af="SOB", process="KHAY")
    result = classify_af(no_khay_inference)
    assert result["segment_code"] == "99"
    assert result["segment_label"] == AF_CONTROL_BY_CODE["99"]["label"]
    ...
    print("AF CONTRACT OK: 19 mã chuẩn, 00/98/99 và no-inference đều đạt.")
    return 0
